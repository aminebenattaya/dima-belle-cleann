...
import { ShoppingBag, Menu, User, LogOut, LogIn, UserPlus, LayoutDashboard } from 'lucide-react';
...
const navItems = [
  ...
  { href: '/style-assistant', label: 'Conseillère IA' },
  { href: '/customer-gallery', label: 'Galerie Clientes' },
  ...
  const renderAuthButtons = (isMobile = false) => {
    ...
    const adminButton = (
        <Button variant="outline" asChild className={isMobile ? "w-full text-left justify-start py-2 text-lg" : ""}>
            <Link href="/amineweldmaryem"><LayoutDashboard className="mr-2 h-4 w-4" />Panneau Admin</Link>
        </Button>
    );

    const profileButton = (
      <Button variant={isMobile ? "ghost" : "ghost"} asChild className={isMobile ? "w-full text-left justify-start py-2 text-lg" : ""}>
        <Link href="/profile"><User className="mr-2 h-4 w-4" />Mon Profil</Link>
      </Button>
    );
