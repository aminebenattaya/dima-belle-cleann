            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-4 lg:grid-cols-5 gap-2">
              {availableImages.map((image, index) => (
                <button
                  ...
                  className={cn(
                    "aspect-square relative rounded-md overflow-hidden border-2 transition-all bg-muted/30",
        ...
        <div className="space-y-6">
          ...
          <div className="text-sm text-muted-foreground prose prose-sm max-w-none">
            ...
            {displayableDetails.length > 0 && (
              <>
                <h3 className="text-md font-semibold mt-4 mb-1 text-foreground">Détails :</h3>
          ...
          {product.colors && product.colors.length > 0 && (
            <div>
              <h3 className="text-md font-semibold mb-2">Couleurs disponibles:</h3>
              <div className="flex flex-wrap gap-2">
                {product.colors.map((color, index) => (
                    <Button
                      ...
                      className={cn(
                        "px-3 py-1 text-sm rounded-md transition-all",
                        {
                          ...
                          "ring-2 ring-destructive ring-offset-2 opacity-75": color.stock === 0,
                        }
                      )}
                    ...
                    >
                      {color.name}
                      {color.stock === 0 && <span className="ml-2"> (Épuisé)</span>}
                    </Button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 space-y-3">
              {selectedColor && (
                  <div className={cn(
                      ...
                      isSelectedColorOutOfStock ? "bg-destructive/10 text-destructive-foreground" : "bg-green-600/10 text-green-800"
                  )}>
                      {isSelectedColorOutOfStock 
                          ? <p className='font-semibold text-destructive'>Cet article est actuellement en rupture de stock.</p>
                          : selectedColor.stock <= 5 
                              ? <p className='font-semibold'>Stock faible ! Plus que {selectedColor.stock} articles disponibles.</p>
                              : <p className='font-semibold text-green-900'>En stock</p>
                      }
                  </div>
              )}
              <div className="flex items-center gap-4">
                  ...
                  <Button size="lg" className="flex-grow" disabled={isOutOfStock} onClick={handleAddToCart}>
                      {isOutOfStock ? (
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction asChild>
              <Link href="/cart">Passer à la caisse</Link>
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
