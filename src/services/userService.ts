 ...
 * This is typically called upon user registration.
 * @param uid The user's unique ID from Firebase Auth.
 * @param data The initial data, usually just email and name.
 */
export async function createUserProfile(uid: string, data: { email: string; fullName: string; }): Promise<void> {
  try {
    const userDocRef = doc(db, USERS_COLLECTION, uid);
 ...
 */
export async function updateUserProfile(uid: string, data: Partial<Omit<UserProfile, 'uid' | 'email'>>): Promise<void> {
  try {
    const userDocRef = doc(db, USERS_COLLECTION, uid);
