...
import { updateSalesAnalyticsOnDelivery } from './statisticsService';
...
export async function deleteOrderAndRestock(orderId: string): Promise<void> {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      // Do not restock items for orders that were already delivered, as the analytics and stock are finalized.
      if (order.status === 'delivered') {
        // Just delete the order document without restocking if it was delivered.
         transaction.delete(orderDocRef);
         console.log(`Order ${orderId} was delivered. Deleting without restocking.`);
         return;
      }
      ...
      for (const item of order.items) {
        ...
        if (productSnap.exists()) {
          ...
          // If a product doesn't exist anymore, we can't restock it, but we log this.
          console.warn(`Produit ${item.productId} non trouvé. Impossible de remettre en stock.`);
        }
      }

      // After updating all product stocks, delete the order document
      transaction.delete(orderDocRef);
    });
    console.log(`Commande ${orderId} supprimée et stock restauré.`);
  } catch (error) {
